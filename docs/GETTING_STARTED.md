...
点击 **开始使用搭手**。页面会告诉你搭手正在为这台电脑准备连接。这一步不需要填写地址、密码或其他连接信息。
...
> 把这 10 张图片原样保存到项目的 `assets/scenes`，不要缩放、转码或降低清晰度，完成后告诉我每张图的字节数和 SHA-256。

搭手会优先保存原始字节。如果 ChatGPT 中的图片链接只对当前浏览器登录状态有效，搭手会明确提示你先下载原文件到已授权文件夹，再继续原样保存；它不会自动改成截图、SVG 包装或压缩图片。
