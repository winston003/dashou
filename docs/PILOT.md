...
1. 使用测试联系人发送的安装包安装 Dashou。
2. 打开 Dashou，点击“开始使用搭手”；页面会显示准备进度，用户不需要填写技术参数。
3. 管理员批准后，页面自动变为“已开通，可以继续”。用户选择一个或多个工作文件夹，点击“开始使用”。
4. 点击“连接 ChatGPT”，跟着客户端的 1/3 → 2/3 → 3/3 打开开发者模式、创建搭手并完成授权；名称、描述、地址和密码都有复制按钮。
5. 客户端显示“ChatGPT 已连接”后，点击“复制任务并打开 ChatGPT”，确认第一个只读项目列表成功。

Dashou 会自动复制连接地址并打开 ChatGPT。普通用户不需要安装额外运行环境，也不需要填写网络参数；ChatGPT 最后的添加和授权仍由用户在自己的账号页面确认。授权页会明确告诉用户回到 Dashou 点击“复制授权密码”。
...
Streamable HTTP、六个工具以及文本本地操作闭环。
...
点击“连接 ChatGPT”会复制连接地址并打开 ChatGPT 官方 Plugins 页面。客户端随后固定显示三个阶段：1/3 打开开发者模式；2/3 复制名称、描述、服务器 URL 并选择 OAuth；3/3 从 Dashou 复制授权密码并允许连接。只有检测到 OAuth 完成和首个 MCP 会话后，客户端才显示“ChatGPT 已连接”；首个只读工具成功后才显示“搭手已经可以干活了”。
