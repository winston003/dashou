...
execFileSync("node", ["scripts/check-control-plane-deploy-compatibility.mjs"], { cwd: repo, stdio: "inherit" });
...
execFileSync("node", ["scripts/check-control-plane-deploy-compatibility.mjs"], {
  cwd: repo,
  stdio: "inherit",
  env: { ...process.env, DASHOU_EXPECT_BUILD_SHA: buildSha },
});
